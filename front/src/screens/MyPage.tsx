				...
				<View style={MyPageLayout.myInfoWrap}>
					...
					<View style={MyPageLayout.myProfileInfoWrap}>
						<View style={MyPageLayout.flexWrap}>
							...
							<TouchableOpacity
								activeOpacity={0.7}
								onPress={() => navigation.navigate("ShowPrivate")}
							>
								<View style={MyPageLayout.editMyPrivate}>
									<Text style={MyPageLayout.editMyPrivateText}>
										개인키 확인하기
									</Text>
								</View>
							</TouchableOpacity>
						...
						<View style={MyPageLayout.myProfileInfo}>
							<Text style={MyPageLayout.myInfo}>
								기분 좋은 오늘 내 반려견과 함께하는{"\n"}
								{`반려견 라이프 스타일, 오늘 산책 한 번 어떠세요 :)`}
							</Text>
							<TouchableOpacity
								activeOpacity={0.7}
								onPress={() => navigation.navigate("EditMyPage")}
							>
								<View style={MyPageLayout.editProfileButton}>
									<Text style={MyPageLayout.editProfileButtonText}>
										정보수정
									</Text>
								</View>
							</TouchableOpacity>
				...
				<View style={MyPageLayout.myPetWrap}>
					...
					<ScrollView style={MyPageLayout.myPetList} horizontal={true}>
						{myDogList.map((myDog, index) => {
							return (
								<MyPet
									bgImg={{ uri: myDog.dogImg }}
									petCreatedDate={myDog.dogBirthDate}
									petName={myDog.dogName}
									petSpecies={myDog.dogBreed}
									dogNo={myDog.dogNo}
								/>
							);
						})}
						...
						>
							<View style={MyPageLayout.addNewMyPetWrap}>
								...
								<Text style={MyPageLayout.addNewMyPetText}>
									반려견 등록하기
								</Text>
							</View>
						</TouchableOpacity>
